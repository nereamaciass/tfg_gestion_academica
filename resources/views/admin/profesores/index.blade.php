    ...
    <table class="w-full table-fixed border-collapse">
        <thead>
            <tr class="border-b text-gray-600">
                <th class="w-20 py-3 px-2 text-center">Foto</th>
                ...
                <th class="py-3 px-2 text-center">
                    <a href="{{ request()->fullUrlWithQuery(['sort' => $direction === 'desc' && $sort === 'email' ? null : 'email', 'direction' => $sort !== 'email' ? 'asc' : ($direction === 'asc' ? 'desc' : ($direction === 'desc' ? 'default' : 'asc'))]) }}" class="inline-flex items-center gap-1 hover:text-blue-600">
                        Email
                ...
                <th class="py-3 px-2 text-center">Teléfono</th>

                <th class="py-3 px-2 text-center">
                    <a href="{{ request()->fullUrlWithQuery(['sort' => $direction === 'desc' && $sort === 'departamento' ? null : 'departamento', 'direction' => $sort !== 'departamento' ? 'asc' : ($direction === 'asc' ? 'desc' : ($direction === 'desc' ? 'default' : 'asc'))]) }}" class="flex justify-center items-center gap-1 hover:text-blue-600">
                        Departamento
                ...
                <th class="py-3 px-2 text-center">Cuenta usuario</th>
        ...
        <tbody class="text-gray-800">
            @foreach($profesores as $prof)
                ...
                <tr class="border-b hover:bg-gray-50">
                    <td class="py-3 pl-0 pr-2 text-center align-middle">
                    ...
                    <td class="py-3 px-2 text-center align-middle">{{ $prof->nombre }}</td>
                    <td class="py-3 px-2 text-center align-middle">{{ $prof->email }}</td>
                    <td class="py-3 px-2 text-center align-middle">{{ $prof->telefono ?? '—' }}</td>
                    <td class="py-3 px-2 text-center align-middle">{{ $prof->departamento ?? '—' }}</td>
                    ...
                    <td class="py-3 px-2 text-center align-middle">
                        <div class="flex justify-center items-center gap-3">
                            <a href="{{ route('admin.profesores.show', $prof) }}" class="w-9 h-9 flex items-center justify-center text-gray-600 hover:text-blue-600" title="Ver">
                            ...
                            <a href="{{ route('admin.profesores.edit', $prof) }}" class="w-9 h-9 flex items-center justify-center text-gray-600 hover:text-yellow-500" title="Editar">
                                <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5 shrink-0" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15.232 5.232l3.536 3.536M9 11l6-6 3 3-6 6H9v-3z"/>
                                </svg>
                            </a>

                            <form id="form-eliminar-{{ $prof->id }}" action="{{ route('admin.profesores.destroy', $prof->id) }}" method="POST" class="m-0 p-0 flex items-center">
                                @csrf
                                @method('DELETE')

                                <button type="button" onclick="confirmarEliminacion({{ $prof->id }})" class="w-9 h-9 flex items-center justify-center text-gray-600 hover:text-red-600" title="Eliminar">
                                    <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5 shrink-0" fill="none" viewBox="0 0 24 24" stroke="currentColor">
